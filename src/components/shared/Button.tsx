...
const Button = ({
  ...
  return (
    <button
      type={type}
      className={`flex w-fit items-center justify-center gap-2 rounded-xl font-bold transition duration-300 disabled:!cursor-not-allowed disabled:opacity-50 ${buttonStyles} ${className} ${buttonSize[size]}`}
      disabled={isDisabled}
      {...props}
    >
      {children}
