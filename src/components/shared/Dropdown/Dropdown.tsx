...
const Dropdown = ({ children, isOpen, position = "right" }: DropdownProps) => {
  ...
  return (
    <ul
      id="dropdown"
      ref={dropDownRef}
      className={`absolute top-full mt-2 w-max min-w-56 origin-top rounded-xl border border-border bg-foreground px-2 py-2 text-base opacity-0 shadow-xl transition-all duration-300 ease-[cubic-bezier(0.87,0,0.13,1)] ${isOpen && "!visible !translate-y-0 !opacity-100"} invisible max-h-48 translate-y-3 overflow-y-auto opacity-0 ${theme === "light" ? "bg-foreground before:bg-foreground" : "bg-muted"} ${dropdownPosition} z-50`}
      onClick={(e) => {
        e.stopPropagation();
      }}
    >
